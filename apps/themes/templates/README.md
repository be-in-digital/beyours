...
| `menu` | `cards` · `zen` · `ledger` · `tickets` · `dotted` — **yes**<br>`mosaic` · `tabs` · `bento` — no | **partly**. `cards` is the grid the page already renders; `zen` is one column with hairline rules; `ledger` numbers the dishes with a CSS counter; `tickets` makes each dish a row with a square thumbnail; `dotted` is the printed carte, two columns of name-leader-price with no photograph at all. The three left want what the engine's card cannot give: `mosaic` and `bento` overlay white text on the dish's own image, and a dish with no photograph gets a placeholder glyph on `--muted`, so they would be unreadable on exactly the dishes nobody has photographed yet — and the contrast sweep cannot see it, because it reads source files and that is text over a photograph. `tabs` wants a sticky category bar that lives in `MenuContent`, not in the grid. An unhonoured value renders `cards`. |
...
templates, 31 name a hero outside the built three, **17** a menu, and **13** a
button style. The menu figure was 26 before `tickets` (5) and `dotted` (4) were
built, leaving `tabs` (8), `bento` (5) and `mosaic` (4); the button figure was
51, the whole catalogue, while none of `btn`'s five was built, and `soft` (14),
