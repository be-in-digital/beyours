...
const workflows: { file: string; workflow: Workflow }[] = fs
  .readdirSync(WORKFLOW_DIR)
  .filter((file) => file.endsWith(".yml") || file.endsWith(".yaml"))
  .sort()
  .map((file) => ({
    file,
    workflow: parse(fs.readFileSync(path.join(WORKFLOW_DIR, file), "utf8")) as Workflow,
  }))
  ...
  for (const { file, workflow } of workflows) {
    for (const [id, job] of Object.entries(workflow.jobs ?? {})) {
      ...
      for (const step of job.steps ?? []) {
        const run = step.run ?? ""
        // A pipeline whose left-hand side can fail. `|| true` is a deliberate
        // discard and says so.
        if (!/\S\s\|\s\S/.test(run) || run.includes("|| true")) continue
        const read =
