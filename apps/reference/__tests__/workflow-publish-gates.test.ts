...
const PUBLISHING_COMMANDS: ReadonlyArray<{ pattern: RegExp; what: string }> = [
  ...
  {
    pattern: /\b(npm|pnpm|yarn)\s+(-r\s+|--recursive\s+)?publish\b/,
    what: 'publishes packages to a registry',
  },
  {
    pattern: /publish-mirror\.mjs(?!.*--check)/,
    what: 'pushes apps/themes to beyours-boilerplate',
  },
...
const OUTWARD_CAPABILITIES: ReadonlyArray<{ pattern: RegExp; what: string }> = [
  {
    pattern: /secrets\.MIRROR_PUSH_TOKEN/,
    what: 'holds MIRROR_PUSH_TOKEN, which can only push to the client mirror',
  },
]
...
describe('every workflow that publishes outside this repository is gated on CI', () => {
  ...
  test.each(publishing)('$file / $id declares needs:', ({ job, effects }) => {
    ...
    expect(needs, `this job ${effects.join(' and ')} — it must wait on a CI job`).not.toHaveLength(
      0,
    )
  })
...
describe('a job waits for every gate its condition reads', () => {
  ...
  test('the scan finds the jobs whose condition reads a dependency', () => {
    expect(conditional.map((entry) => `${entry.file}:${entry.id}`)).toEqual([
      'e2e.yml:e2e-report',
      // Both read `needs.hold.outputs.held` and stand down while publication is
      // deliberately held. They measure distance from the registry — is the
      // boilerplate current, is a merged bump unpublished — and under a hold
      // both answers are "no" BY DESIGN, so a daily issue on each would report
      // the decision back to the people who took it.
      'mirror-health.yml:stale',
      'mirror-health.yml:release-waiting',
      'publish-mirror.yml:publish',
...
type Scenario = {
  ...
  plan?: JobResult
  /**
   * `needs.plan.outputs.held` — is `RELEASE_HOLD.md` in the tree?
   *
   * An OUTPUT, not a result, and the difference is why it is modelled
   * separately: `plan` can succeed perfectly and still answer "do not publish".
   * Defaults to unheld, so every scenario written before the hold existed keeps
...
function evaluate(expression: string, scenario: Scenario): boolean {
  const context: Record<string, string> = {
    ...
    // A string, because that is what a job output IS — `held=true` written to
    // $GITHUB_OUTPUT arrives as the text "true", which is why the workflow
    // compares against a quoted literal rather than testing truthiness. An
    // output that was never set arrives as the empty string, and the condition
    // reads that as unheld: the hold is asserted, never assumed.
    'needs.plan.outputs.held': scenario.held === true ? 'true' : 'false',
  }
...
describe('the mirror publishes on exactly the runs that passed CI', () => {
  ...
  const cases: Array<[string, Scenario, boolean]> = [
    ...
    ).map((conclusion): [string, Scenario, boolean] => [
...
describe('the release publishes on exactly the runs that passed CI', () => {
  ...
  const cases: Array<[string, Scenario, boolean]> = [
    ...
    // ── The release hold (RELEASE_HOLD.md) ───────────────────────────────
    //
    // Everything green, everything gated, nothing wrong — and it must still
    // not publish. A published npm version cannot be withdrawn, so the one
    // direction this gate may never fail in is "published anyway".
    ['a push that is green in every way, but held', push({ held: true }), false],
    [
      // The hold outranks the 96% case too. `e2e: skipped` is what an ordinary
      // push looks like, and it is exactly the shape a rename lands in.
      'a push with nothing to gate, but held',
      push({ e2e: 'skipped', held: true }),
      false,
    ],
    [
      // And the other direction, so the gate is not simply always-off: with the
      // file gone this is the same scenario as the first case in this table.
      'the same push once the hold is lifted',
      push({ held: false }),
      true,
    ],
    ...(['failure', 'cancelled', 'skipped'] as const).map((plan): [string, Scenario, boolean] => [
...
describe('a called workflow never asks for more than its caller grants', () => {
  ...
  const localCalls = WORKFLOW_FILES.flatMap((file) => {
    ...
    return Object.entries(wf.jobs ?? {})
      .filter(
        ([, job]) => typeof job.uses === 'string' && job.uses.startsWith('./.github/workflows/'),
      )
      .map(([jobName, job]) => ({
        file,
        jobName,
  ...
  test.each(localCalls)('$file job $jobName calling $called', ({ called, granted }) => {
...
describe('a step that reads tags it did not create fetches them first', () => {
  ...
  const readsForeignTags = RAW_WORKFLOWS.filter(
    ({ code }) =>
      /git tag\s+--points-at/.test(code) && !/changeset publish|git tag\s+-[am]/.test(code),
  )
