 ...
 * bundle cost beyond this file. That is the state of every CI build and of
 * local development. See `apps/docs/deployment/sentry.md`.
 */
import * as Sentry from "@sentry/nextjs";
import { resolveSentryOptions } from "@/lib/observability/sentry";
...
const options = resolveSentryOptions("browser", {
  ...
  // whole `process.env` object to a function in another module reads an empty
  // object in the browser: Sentry would resolve to `null` and stay silently
  // off — the exact failure this integration exists to end.
  ...
  // default. With it off, set NEXT_PUBLIC_SENTRY_ENVIRONMENT instead, or every
  // preview deploy files its errors under `production`.
