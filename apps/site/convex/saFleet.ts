...
export const updateStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    /* What go-live does NOT do is promote `health` from "unknown" to
       "healthy". It used to, and that was an assertion about a system nobody
       had contacted — the console then showed a measured-looking word for a
       deployment that had never answered a request. `health` is written only
       by the prober now (convex/saMonitoring.ts) and stays "unknown" until it
       has run, which is at most ten minutes. Do not add it back here: the
       whole point of the prober is that this column means something. */
    if (args.status === "live") {
