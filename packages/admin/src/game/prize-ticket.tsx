...
import type { PrizeTicketProps } from "./api-contract"
...
export function PrizeTicket({ code, api }: PrizeTicketProps) {
  const ticket = useQuery(api.getRedemptionByCode, code ? { code } : "skip") as
    | TicketData
    | null
    | undefined
  const canRedeem = useQuery(api.canRedeem, code ? { code } : "skip") as
    | boolean
    | undefined
  const redeemMutation = useMutation(api.redeemByCode)
