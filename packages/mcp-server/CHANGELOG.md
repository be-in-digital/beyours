...
- Report the versions this release actually cuts.
  ...
  supersedes. It names `admin@22.0.1`, `convex-functions@7.6.1` and
  `convex-schema@6.8.1`; this release cuts **23.0.0**, 7.7.0 and 6.9.0.
...
- Carry the versions this release cut
  ...
  and is `mcp-server`'s own source, so bumping `admin`, `convex-functions`,
  `convex-schema` and `restaurant` changes it. Without a release of its own the
  table would name 22.0.1 while every client installing `mcp-server` still
  received the build naming 22.0.0 — the registry serving a version index a
  release behind the versions it indexes.

  `check:source-drift` is what refuses that, and it refused this release until
  this changeset existed. `registry.test.ts` is what caught the drift in the
  first place, by comparing every generated version against its manifest.
...
- Carry the versions the platform-controls release cut

  `src/package-versions.ts` is generated by `sync:versions` and states which version
  of each engine package the registry serves. `convex-schema` and `admin` moved, so
  this moves with them — otherwise the registry answers with numbers one release
  behind, and `registry.test.ts` is what notices.

## 1.1.10

### Patch Changes

- Carry the versions the stock-ledger release cut

  `src/package-versions.ts` is generated by `sync:versions` and states which version
  of each engine package the registry serves. `convex-functions` and `convex-schema`
  moved, so this moves with them — otherwise the registry answers with numbers one
  release behind, and `registry.test.ts` is what notices.

## 1.1.9

### Patch Changes

- Carry the versions the order-tracking release cut

  `src/package-versions.ts` is generated by `sync:versions` and states which
  version of each engine package the registry serves. `core` and
  `convex-functions` moved for #96, so this moves with them — otherwise the
  registry answers with numbers that are one release behind, and
  `registry.test.ts` is what notices.
