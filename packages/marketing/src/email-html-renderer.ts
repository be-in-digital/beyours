 ...
 *
 * Tracking (open/click) is handled natively by AWS SES Configuration Set.
...
/**
 * Blocks allowed inside columns (no recursive nesting).
 */
...
export function renderProductBlock(
  ...
  if (block.layout === "grid" && products.length >= 2) {
    // 2-column grid using nested tables
    const rows: string[] = []
...
/**
 * Render a single child block inside a column.
 * Uses the same render functions, but no product data is passed.
 */
 ...
 * For ProductBlock, productData must be provided externally.
 */
...
/**
 * Render a complete email template to a standalone HTML document.
 * Includes header (logo), body (blocks), footer (unsubscribe, branding).
 */
