...
- Renamed from `@be-in-digital/convex-functions` to `@be-yours/convex-functions`, and reset to 1.0.0.
  ...
  from `7.7.0`. The old scope keeps everything it published: those
...
- 39fe018: Refuse an article filed under another establishment's rubric
...
- 6a1595a: Stop a public coupon lookup publishing the campaign's budget
...
- 9ff9978: Stop backing up sixteen tables that cannot hold a row
...
- ff0588c: Stop an anonymous CMS read saying who is editing the page
...
- d0a7fa6: Tell the diner their order is ready, and let the dining-room screen be opened
...
- 4a648b5: Let every card provider be asked what became of a checkout.
...
- 6bab077: Make a catalogue translation run observable.
...
- 0419711: Build the customer book, and give the Clients screen something to show
...
- f5edeca: Stop an erasure report asking for a check that cannot find anything.
...
paymentConnections convex/stripe.ts` returned 0 — the charge path never read the
  ...
  New subpath `@be-yours/convex-functions/stripeChargeRouting` exports
...
- 7ae8072: `convex` moves from a hard dependency to a peer dependency.
  ...
  **Breaking: consumers must declare `convex` themselves**, at `^1.44.0`. Every
  ...
  |                                                       | Result                                                                                                                             |
  | ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
  | **Before** — `dependencies: { convex: "1.44.0" }`     | installs quietly, **two copies**: the consumer's `convex@1.42.0` and a nested `@be-yours/convex-schema/node_modules/convex@1.44.0` |
  | **After** — `peerDependencies: { convex: "^1.44.0" }` | npm **refuses**: `npm error peer convex@"^1.44.0" from @be-yours/convex-schema@2.2.0`                                              |
...
- 74de4e9: `orderConfirmation` and `displayConfig` are gone; `soundConfig` stays.
  ...
  route `/dashboard/settings` and `/dashboard/stores/[id]` to `@be-yours/admin`,
...
- b8aaa34: Rename package scope from @beindigital-engine to @be-yours for GitHub Packages compatibility
...
All notable changes to `@be-yours/convex-functions` will be documented in this file.
